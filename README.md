...
Pookie lets a laptop join a WebOps task mesh, receive MCP-style execution requests, run an AI browser worker, and return a proof hash. This repo is intentionally hackathon-shaped: the P2P, payment, browser, and storage seams are all visible so judges can understand the full DePIN flow quickly.
...
- 0G Storage upload is a clearly marked stub in `src/server.js`.
